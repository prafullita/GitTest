	...
	public Image addImage(@RequestPart Image img,@RequestPart MultipartFile file,@RequestPart String albumaName)
	{
		...
		try {
			if (file.isEmpty()) {
				...
				System.out.println(ServletUriComponentsBuilder.fromCurrentContextPath().path("/admin/Images//"+albumaName).path(file.getOriginalFilename()).toUriString());
				imgHelper.uploadFile(file,albumaName);		
				
				img.setImgUrl(ServletUriComponentsBuilder.fromCurrentContextPath().path("/admin/Images//"+albumaName).path(file.getOriginalFilename()).toUriString());
			}
		} catch (Exception ee) {
